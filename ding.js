...
const popupRepoAdd = async () => {
	...
	const close = popup(dom.h1('New repository'), dom.form(async function submit(e) {
		...
		close();
	}, fieldset = dom.fieldset(dom.div(style({ display: 'grid', columnGap: '1em', rowGap: '.5ex', gridTemplateColumns: 'min-content 1fr', alignItems: 'top' }), 'VCS', vcs = dom.select(dom.option('git'), dom.option('mercurial'), dom.option('command'), vcsChanged), 'Origin', originBox = dom.div(originInput = origin = dom.input(attr.required(''), attr.placeholder('https://... or ssh://... or user@host:path.git'), style({ width: '100%' }), function keyup() {
		if (nameChanged) {
			return;
		}
		...
		s = s.replace(/\.git$/, '');
		name.value = s;
	})), 'Name', name = dom.input(attr.required(''), function change() { nameChanged = true; }), dom.div('Default branch', style({ whiteSpace: 'nowrap' })), defaultBranch = dom.input(attr.value('main'), attr.placeholder('main, master, default'), function change() { branchChanged = true; }), dom.div(), dom.label(reuseUID = dom.input(attr.type('checkbox'), attr.checked('')), ' Reuse $HOME and UID for builds for this repo', attr.title('By reusing $HOME and running builds for this repository under the same UID, build caches can be used. This typically leads to faster builds but reduces isolation of builds.'))), dom.br(), dom.p('The build script can be configured after creating.'), dom.div(style({ textAlign: 'right' }), dom.submitbutton('Add')))));
	originInput.focus();
};
const pageHome = async () => {
	...
	const render = () => {
		...
		dom._kids(pageElem, dom.div(style({ marginBottom: '1ex' }), dom.a(attr.href('#toolchains'), 'Toolchains'), ' '), dom.div(style({ marginBottom: '1ex' }), dom.clickbutton('Add repo', attr.title('Add new repository, to build.'), function click() {
			...
			return (rb.Builds || []).map((b, i) => dom.tr(i === 0 ? dom.td(dom.a(rb.Repo.Name, attr.href('#repo/' + encodeURIComponent(rb.Repo.Name))), attr.rowspan('' + (rb.Builds || []).length)) : [], dom.td(b.Branch), dom.td(dom.a('' + b.ID, attr.href('#repo/' + encodeURIComponent(rb.Repo.Name) + '/build/' + b.ID))), dom.td(buildStatus(b)), dom.td(b.Start ? atexit.age(b.Start, b.Finish || undefined) : ''), dom.td(b.Version), dom.td(formatCoverage(rb.Repo, b)), dom.td(formatBuildSize(b)), dom.td(rb.Repo.UID ? dom.span(formatSize(rb.Repo.HomeDiskUsage), attr.title('Of reused home directory')) : []), dom.td(atexit.ageMins(b.Created, undefined)), dom.td(style({ textAlign: 'left' }), buildErrmsg(b))));
		}))));
	};
	render();
...
const pageToolchains = async () => {
	...
	const render = () => {
		...
		let gocur;
		let goprev;
		dom._kids(pageElem, dom.p('Go toolchains can easily be installed in the toolchains directory set in the configuration file. Build scripts can add $toolchaindir/<goversion>/bin to their $PATH.'), dom.h1('Current and previous Go toolchains'), dom.p('The "current" Go toolchain is available through $toolchaindir/go/bin, and the "previous" Go toolchain through $toolchaindir/go-prev/bin.'), dom.table(dom.tr(dom.td('Current'), dom.td(dom.form(async function submit(e) {
			e.stopPropagation();
			e.preventDefault();
			...
			render();
		}, dom.fieldset(goprev = dom.select(dom.option('(none)', attr.value('')), installed.map(s => dom.option(s, active['go-prev'] === s ? attr.selected('') : []))), ' ', dom.submitbutton('Set', attr.title('Set Go toolchain as "go-prev"'))))))), dom.br(), dom.h1('Released and installed toolchains'), dom.div(dom.ul(style({ lineHeight: '1.75' }), groups.map(g => dom.li(g.map(s => [
...
const docsBuildScript = () => {
	return dom.div(dom.h1('Build script environment'), dom.p('The build script is run in a clean environment. It should exit with status 0 only when successful. Patterns in the output indicate where build results can be found, such as files and test coverage.'), dom.p('The working directory is set to $DING_BUILDDIR/checkout/$DING_CHECKOUTPATH.'), dom.h2('Example'), dom.pre(`#!/bin/bash
set -xeuo pipefail
...
const pageRepo = async (repoName) => {
	...
	const renderBuilds = () => {
		atexit.run();
		dom._kids(buildsElem, dom.h1('Builds'), dom.table(dom._class('striped', 'wide'), dom.thead(dom.tr(['ID', 'Branch', 'Status', 'Duration', 'Version', 'Coverage', 'Disk usage', 'Age'].map(s => dom.th(s)), dom.th(style({ textAlign: 'left' }), 'Error'), dom.th('Actions'))), dom.tbody(builds.length === 0 ? dom.tr(dom.td(attr.colspan('10'), 'No builds', style({ textAlign: 'left' }))) : [], builds.map(b => dom.tr(dom.td(dom.a('' + b.ID, attr.href('#repo/' + encodeURIComponent(repo.Name) + '/build/' + b.ID))), dom.td(b.Branch), dom.td(buildStatus(b)), dom.td(b.Start ? atexit.age(b.Start, b.Finish || undefined) : ''), dom.td(b.Version), dom.td(formatCoverage(repo, b)), dom.td(formatBuildSize(b)), dom.td(atexit.ageMins(b.Created, undefined)), dom.td(style({ textAlign: 'left' }), buildErrmsg(b)), dom.td(dom.clickbutton('Rebuild', attr.title('Start new build.'), async function click(e) {
...
const pageBuild = async (repoName, buildID) => {
	...
	const render = () => {
		...
		dom._kids(pageElem, dom.div(style({ marginBottom: '1ex' }), dom.clickbutton('Remove build', b.Released ? attr.disabled('') : [], attr.title('Remove this build completely from the file system and database.'), async function click(e) {
			...
			render();
		})), dom.div(dom.h1('Summary'), dom.table(dom.tr(['Status', 'Branch', 'Duration', 'Commit', 'Version', 'Coverage', 'Size', 'Age'].map(s => dom.th(s)), dom.th(style({ textAlign: 'left' }), 'Error')), dom.tr(dom.td(buildStatus(b)), dom.td(b.Branch), dom.td(b.Start ? atexit.age(b.Start, b.Finish || undefined) : ''), dom.td(b.CommitHash), dom.td(b.Version), dom.td(formatCoverage(repo, b)), dom.td(formatBuildSize(b)), dom.td(atexit.ageMins(b.Created, undefined)), dom.td(style({ textAlign: 'left' }), b.ErrorMessage ? dom.div(b.ErrorMessage, style({ maxWidth: '40em' })) : [])))), dom.br(), dom.div(style({ display: 'grid', gap: '1em', gridTemplateColumns: '1fr 1fr', justifyItems: 'stretch' }), dom.div(dom.h1('Steps'), stepsBox = dom.div(stepViews = steps.map((step) => newStepView(step)))), dom.div(dom.div(dom.div(style({ display: 'flex', gap: '1em' }), dom.h1('Results'), b.Status === api.BuildStatus.StatusSuccess ? dom.div(link('dl/' + (b.Released ? 'release' : 'result') + '/' + encodeURIComponent(repo.Name) + '/' + b.ID + '/' + encodeURIComponent(repo.Name) + '-' + b.Version + '.zip', 'zip'), ' ', link('dl/' + (b.Released ? 'release' : 'result') + '/' + encodeURIComponent(repo.Name) + '/' + b.ID + '/' + encodeURIComponent(repo.Name) + '-' + b.Version + '.tgz', 'tgz')) : []), dom.table(dom.thead(dom.tr(['Name', 'OS', 'Arch', 'Toolchain', 'Link', 'Size'].map(s => dom.th(s)))), dom.tbody(results.length === 0 ? dom.tr(dom.td(attr.colspan('6'), 'No results', style({ textAlign: 'left' }))) : [], results.map(rel => dom.tr(dom.td(rel.Command), dom.td(rel.Os), dom.td(rel.Arch), dom.td(rel.Toolchain), dom.td(link((b.Released ? 'release/' : 'result/') + encodeURIComponent(repo.Name) + '/' + b.ID + '/' + (b.Released ? basename(rel.Filename) : rel.Filename), rel.Filename)), dom.td(formatSize(rel.Filesize))))))), dom.br(), dom.div(dom.h1('Build script'), dom.pre(b.BuildScript)))));
	};
	render();
	page.subscribe(streams.build, (e) => {
		if (e.RepoName !== repo.Name) {
